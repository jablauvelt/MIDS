...
-- I average scores from all measures together for each state, but I do not include measures
-- that do not have a sample size available or that have a sample size less than 35.
SELECT State, (AVG(Effective_Score) + AVG(Readmission_Score) + AVG(Complications_Score)) / 3 AS Final_Rating
FROM (
