...
LOAD DATA INPATH '/user/w205/hospital_compare/readmissions/readmissions.csv' OVERWRITE INTO TABLE readmissions_raw;
...
FROM	(SELECT Measure_ID, CAST(CASE WHEN Score = 'Not Available' THEN NULL
         ...
         FROM readmissions_raw) tbl
GROUP BY Measure_ID;


-- Create readmissions table that is ready for later analysis. It includes the score_relative field, which
